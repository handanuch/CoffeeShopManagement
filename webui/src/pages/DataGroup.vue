<template>
  <WideDiv>
    <WideDivTitle :title="t('data_group')" :badge="$label" />
...
import lang from "@/locales/DataGroup.json";
...
const viewData = () => {
  $api.post("/api/dataGroup/viewdata", dto, (data) => {
    datalist.value = data;
  });
};
...
const historyData = (x) => {
  ...
  dto.DTG_ID = x.DTG_ID;
  datahistory.value = [];
  $api.post("/api/dataGroup/getstm", dto, (data) => {
